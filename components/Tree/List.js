...
const TreeList = () => {
    ...
    const { trees, showList, setShowList } = useContext(ScreenContext);
    ...
    const handleChange = (e) => {
        ...
        const list = trees.features.find(tree => +tree.properties.treeID == e.nativeEvent.text);
        if (list === undefined) { 
            setTreeList([{properties: {treeID: "No Tree Found"}}])
            return;
        }
        setTreeList([list]);
        console.log(list);
                        ...
                        <>
                            <View className="absolute top-28 px-2 drop-shadow-2xl w-2/3">
                                <ScrollView className="w-full h-fit max-h-[400px] px-2 bg-slate-400 rounded-b-xl ">
                                    {search !== "" ? treeList.map((tree, index) => (
                                    <View key={index} className="flex flex-row w-full border-t-2 border-gray-600 justify-between items-center">
                                        <Text className="font-bold pl-1 py-2 text-lg text-white">{tree.properties.treeID.toString().padStart(4, '0')}</Text>
                                    </View>
                                    )
